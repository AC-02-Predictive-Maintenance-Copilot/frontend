...
function UserManagementPage() {
  const { users, loading, handleVerifyUser, handleUnverifyUser, handleDeleteUser } = useUsers();
  ...
  const handleUnverify = async (userId) => {
    ...
    await handleUnverifyUser(userId);
    setActionLoading(null);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <HeaderText title="User Management" desc="Manage engineer accounts and permissions" />
        ...
        >
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Registered engineers</p>
            </CardContent>
          </Card>
        </motion.div>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {user.isVerified ? (
                              ...
                              >
                                ...
                                {actionLoading === `unverify-${user.id}` ? "..." : "Unverify"}
                              </Button>
                            ) : (
                              <Button
                                variant="outline"
                              ...
                              >
                                ...
                                {actionLoading === `verify-${user.id}` ? "..." : "Verify"}
                              </Button>
                            )}
                            <Button
                              variant="outline"
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open) => !open && setDeleteDialog({ open: false, userId: null, userName: "" })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{deleteDialog.userName}</strong>? This action cannot be undone and will permanently remove the user account and all associated data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
